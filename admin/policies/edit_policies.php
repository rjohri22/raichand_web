...
$categories = $title = $policies_categories = $description = "";
if (isset($_GET['id'])) {
    $q  = "SELECT * FROM cat where id = " . $_GET['id'];
    $item = $con->query($q);
...
if (isset($_POST['submit'])) {
    ...
    //  $sql = "UPDATE bname_category set `bname` = '$bname',`filename`='$upload_image' where id = " . $_GET['id']; 
     ...
     if($upload_image == ""){
        $sql = "UPDATE cat set `categories` = '$categories', `bname_order` = $order where id = " . $_GET['id']; 

        }
    else{
        // echo "else";exit;
        move_uploaded_file($temp_name,  $_SERVER['DOCUMENT_ROOT'].'/admin/policies/'.'image/'.$_FILES["uploadfile"]["name"]);
        $sql = "UPDATE cat set `categories` = '$categories',`bname_order` = $order,`filename`='$upload_image' where id = " . $_GET['id']; 
            ...
            <div class="container" style="margin-top: 20px">
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <input type="text" class="form-control" name="categories" value="<?php echo $rm['categories']; ?>" required><br>
